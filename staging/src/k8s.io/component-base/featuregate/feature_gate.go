...
type FeatureSpec struct {
	...
	// MinCompatibilityVersion indicates the lowest control plane version that this feature stage is compatible with.
	// If a server's --min-compatibility-version is less than this value, this feature stage will be skipped.
	// This is useful for features with compatibility implications (like a new API field),
	// allowing them to be introduced as Beta but only take effect when all control plane components are upgraded.
	// If unspecified, it is inherited from the previous feature stage.
	//
	// Version vs. MinCompatibilityVersion:
	//  - Version determines the availability and default value of the feature stage based on the server's emulation version.
	...
	//  - With --min-compatibility-version=1.34 (default), FeatureA is Alpha and disabled by default.
	//  - With --min-compatibility-version=1.35, FeatureA becomes Beta and enabled by default.
		...
		for i, spec := range specs {
			...
			if spec.MinCompatibilityVersion != nil {
				if len(spec.MinCompatibilityVersion.Components()) != 2 {
					return fmt.Errorf("feature %q specified patch min compatibility version: %s", name, spec.MinCompatibilityVersion)
				}
			...
			if i > 0 {
				...
				if spec.MinCompatibilityVersion == nil && lastSpec.MinCompatibilityVersion != nil {
					spec.MinCompatibilityVersion = lastSpec.MinCompatibilityVersion
					specs[i] = spec
				}
